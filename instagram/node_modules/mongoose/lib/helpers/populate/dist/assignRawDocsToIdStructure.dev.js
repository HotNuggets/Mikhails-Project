...
function assignRawDocsToIdStructure(rawIds, resultDocs, resultOrder, options, recursed) {
  ...
  var newOrder = [];
  var sorting = options.isVirtual && options.justOne && rawIds.length > 1 ? false : options.sort && rawIds.length > 1;
  ...
  for (i = 0; i < len; ++i) {
    ...
    if (options.clone && doc != null) {
      if (options.lean) {
        var _model = leanPopulateMap.get(doc);
    ...
    if (recursed) {
      if (doc) {
        if (sorting) {
          var _resultOrder = resultOrder[sid];
