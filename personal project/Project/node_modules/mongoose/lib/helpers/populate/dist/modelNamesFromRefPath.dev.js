...
module.exports = function modelNamesFromRefPath(refPath, doc, populatedPath, modelSchema, queryProjection) {
  ...
  if (hasNumericPropRE.test(populatedPath)) {
    var chunks = populatedPath.split(hasNumericPropRE);
    ...
    var _refPath = '';
    var _remaining = refPath; // 2nd, 4th, etc. will be numeric props. For example: `[ 'a', '.0.', 'b' ]`
    ...
    var _refValue = mpath.get(_refPath, doc, lookupLocalFields);

    var _modelNames = Array.isArray(_refValue) ? _refValue : [_refValue];

    _modelNames = utils.array.flatten(_modelNames);
    return _modelNames;
  }

  var refValue = mpath.get(refPath, doc, lookupLocalFields);
  var modelNames;
