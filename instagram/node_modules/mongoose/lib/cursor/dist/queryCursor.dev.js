    ...
    if (err != null) {
      if (err instanceof kareem.skipWrappedFunction) {
        ...
        if (resultValue != null && (!Array.isArray(resultValue) || resultValue.length)) {
          var _err = new MongooseError('Cannot `skipMiddlewareFunction()` with a value when using ' + '`.find().cursor()`, value must be nullish or empty array, got "' + util.inspect(resultValue) + '".');

          _this._markError(_err);

          _this.listeners('error').length > 0 && _this.emit('error', _err);
          return;
        }
...
QueryCursor.prototype.addCursorFlag = function (flag, value) {
  ...
  _waitForCursor(this, function () {
    _this6.cursor.addCursorFlag(flag, value);
  });
...
function _onNext(error, doc) {
  ...
  if (this.ctx._batchDocs.length < this.ctx.options._populateBatchSize) {
    ...
    if (this.ctx._batchDocs.length > 0 && this.ctx._batchDocs.length % 1000 === 0) {
      return immediate(function () {
        return _this7.ctx.cursor.next().then(function (res) {
          _onNext.call(_this7, null, res);
        }, function (err) {
          _onNext.call(_this7, err);
        });
      });
    }
...
function _populateBatch() {
  ...
  this.ctx.query.model.populate(this.ctx._batchDocs, this.ctx._pop).then(function () {
    _nextDoc(_this8.ctx, _this8.ctx._batchDocs.shift(), _this8.ctx._pop, _this8.callback);
  }, function (err) {
    _this8.callback(err);
  });
}
