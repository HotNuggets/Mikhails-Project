...
var omitUndefined = require('../omitUndefined');
...
var arithmeticOperatorNumber = new Set(['$abs', '$exp', '$ceil', '$floor', '$ln', '$log10', '$sqrt', '$sin', '$cos', '$tan', '$asin', '$acos', '$atan', '$atan2', '$asinh', '$acosh', '$atanh', '$sinh', '$cosh', '$tanh', '$degreesToRadians', '$radiansToDegrees']);
...
function _castExpression(val, schema, strictQuery) {
  ...
  if (val.$cond != null) {
    ...
    if (Array.isArray(val.$switch.branches)) {
      val.$switch.branches = val.$switch.branches.map(function (v) {
        return _castExpression(v, schema, strictQuery);
      });
    }
  ...
  if (val.$round) {
    ...
    val.$round = $round.map(function (v) {
      return castNumberOperator(v, schema, strictQuery);
    });
  }
