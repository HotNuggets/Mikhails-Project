...
module.exports = function getSchemaTypes(model, schema, doc, path) {
  ...
  var discriminatorKey = schema.discriminatorMapping && schema.discriminatorMapping.key;

  if (discriminatorKey && model != null) {
    if (doc != null && doc[discriminatorKey] != null) {
      var discriminator = getDiscriminatorByValue(model.discriminators, doc[discriminatorKey]);
      schema = discriminator ? discriminator.schema : schema;
    } else if (model.discriminators != null) {
      return Object.keys(model.discriminators).reduce(function (arr, name) {
        var disc = model.discriminators[name];
  ...
  function search(parts, schema, subdoc, nestedPath) {
    ...
    while (p--) {
        ...
        if (foundschema.schema != null && foundschema.schema.discriminators != null) {
          (function () {
            ...
            schemas = Object.keys(discriminators).reduce(function (cur, discriminator) {
              var tiedValue = discriminators[discriminator].discriminatorMapping.value;
      ...
      if (topLevelDoc != null && topLevelDoc.$__ && topLevelDoc.$populated(fullPath) && p < parts.length) {
        var _model = doc.$__.populated[fullPath].options[populateModelSymbol];

        if (_model != null) {
          var _ret4 = search(parts.slice(p), _model.schema, subdoc ? mpath.get(trypath, subdoc) : null, nestedPath.concat(parts.slice(0, p)));
      ...
      if (_val != null) {
        var _model2 = Array.isArray(_val) && _val.length > 0 ? leanPopulateMap.get(_val[0]) : leanPopulateMap.get(_val); // Populated using lean, `leanPopulateMap` value is the foreign model


        var _schema3 = _model2 != null ? _model2.schema : null;

        if (_schema3 != null) {
          var _ret5 = search(parts.slice(p), _schema3, subdoc ? mpath.get(trypath, subdoc) : null, nestedPath.concat(parts.slice(0, p)));

          if (_ret5 != null) {
            _ret5.$parentSchemaDocArray = _ret5.$parentSchemaDocArray || (_schema3.$isSingleNested ? null : _schema3);
            return _ret5;
          }
        }
      }
